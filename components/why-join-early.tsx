...
function LiveCount() {
  ...
  return (
    <div className="mt-12 inline-flex items-center gap-2.5 bg-white border border-[#e8edf3] rounded-full px-5 py-2.5 text-sm text-[#475569] shadow-sm">
...
export default function WhyJoinEarly() {
  return (
    <section className="bg-[#f8fafc] py-24 border-y border-[#eef2f7]">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {PERKS.map(({ icon: Icon, title, desc }) => (
            <div
              key={title}
              className="bg-white border border-[#e8edf3] rounded-2xl p-8 hover:shadow-md transition-shadow duration-200"
            >
              <div className="w-11 h-11 rounded-xl bg-[#0f2a44] flex items-center justify-center mb-5">
                <Icon className="w-5 h-5 text-[#0ea5e9]" strokeWidth={1.7} />
              </div>
              <h3 className="text-lg font-semibold text-[#0f2a44] mb-2.5">{title}</h3>
