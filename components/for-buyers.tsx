  ...
  return (
    <section className="bg-[#f8fafc] py-16 sm:py-24">
      <div className="max-w-7xl mx-auto px-5 sm:px-6">
        ...
        <div className="grid md:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
          {STEPS.map(({ icon: Icon, step, title, desc }, i) => (
            <div key={step} className="relative">
              ...
              <div className="relative bg-white border border-[#e8edf3] rounded-2xl p-5 sm:p-7 h-full">
                <div className="flex items-center gap-3 mb-4">
                  <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-[#0f2a44] flex items-center justify-center shrink-0">
                    <Icon className="w-5 h-5 sm:w-6 sm:h-6 text-[#0ea5e9]" strokeWidth={1.6} />
                  </div>
                  <span className="text-xl sm:text-2xl font-bold text-[#e2e8f0]">{step}</span>
                </div>
                <h3 className="text-base sm:text-lg font-semibold text-[#0f2a44] mb-2">{title}</h3>
        ...
        <a href="#waitlist-form" className="mt-8 sm:mt-10 inline-flex items-center gap-2 text-[#0ea5e9] font-semibold hover:gap-3 transition-all">
